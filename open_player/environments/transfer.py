...
World B (narrow corridors, scattered resources, fast enemies) is the
zero-shot / adaptation test world.  They differ structurally, not just by
seed.
"""
...
def make_transfer_envs(config: Any) -> TransferPair:
    """Build World A / World B from config.transfer.world_a / world_b."""
    env_a = _env_for(config, config.get("transfer.world_a", {}))
    env_b = _env_for(config, config.get("transfer.world_b", {}))
    return TransferPair(world_a=env_a, world_b=env_b)
